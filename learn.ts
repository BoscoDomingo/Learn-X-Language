...
// Optional chaining
//Similar to Swift, but TS is a bit more strict since it has to check for both null and undefined values and might throw errors even if one is checked
...
const resultWithOptional = item?.value + 1; //Still an error since item?.value is still possibly undefined
const resultWithOperator = item!.value + 1; //Ah, that's the one!

// Non-null Assertion operator
function assertingNonNull(maybeString: string | undefined | null): void {
...
function assertingNonNull2(numGenerator: () => number | undefined) {
...
// Careful, because it can still fail at runtime even if it checks out at compilation time
assertingNonNull2(undefined); // runtime error: Uncaught TypeError: numGenerator is not a function
...
var p3: Person = p2; // Ok, read-only alias for p2, copied by reference
