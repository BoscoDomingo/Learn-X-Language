...
can include line breaks.` // Multi-line string.
...
// Pointers ELI5
// * next to a type: indicates a pointer to that type.
// * next to a variable: follows through a pointer (cannot be used with non-pointer variables)
// & next to a variable: indicates the address of that variable.

func printer(input *int) {
	fmt.Println(*input)
}

func learnPointersEasily() {
	a := 4 // int type
	b := &a // *int type
	fmt.Println(*b) // 4
...
func learnFlowControl() {
	...
	/*
		The interface{} type describes an interface with zero methods.
		Every Go type implements at least zero methods and therefore satisfies
		the empty interface, making it a general container type.
	*/
	...
	// x == 42 here since the for loop modified it in its own scope.
...
func learnDefer() (ok bool) {
	// A defer statement pushes a function call onto a stack.
